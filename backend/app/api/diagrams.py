...
async def list_diagrams() -> list[dict]:
    ...
    try:
        ...
        logger.exception("Failed to list diagrams: %s", e)
...
async def create_diagram(payload: DiagramCreate) -> DiagramResponse:
    ...
    try:
        ...
        logger.exception("Failed to create diagram: %s", e)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error")
...
async def get_diagram(diagram_id: UUID) -> DiagramResponse:
    ...
    try:
        ...
        logger.exception("Failed to get diagram %s: %s", diagram_id, e)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error")
...
async def update_diagram(diagram_id: UUID, payload: DiagramUpdate) -> DiagramResponse:
    ...
    try:
        ...
        logger.exception("Failed to update diagram %s: %s", diagram_id, e)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error")
...
async def delete_diagram(diagram_id: UUID) -> None:
    ...
    try:
        ...
        logger.exception("Failed to delete diagram %s: %s", diagram_id, e)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error")
